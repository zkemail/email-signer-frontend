...
export default function HashApproval({ email, setEmail, accountCode, setAccountCode, walletClient, walletAddress }: HashApprovalProps) {
    ...
    const getTemplateId = async (signerAddress: string) => {
        ...
        try {
            ...
            addLog(`Error getting template ID: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    };
    ...
    const requestSignature = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Failed to request signature: ${await response.text()}`);
            }
            ...
            addLog(`Error requesting signature: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    };
            ...
            try {
                ...
                if (status.response) {
                    ...
                    try {
                        ...
                        addLog(`Error approving hash: ${error instanceof Error ? error.message : String(error)}`);
                        setResult({
                            success: false,
                ...
                addLog(`Error polling for proof: ${error instanceof Error ? error.message : String(error)}`);
                retries++;
            }
        }, 2000);
        ...
        <div className="w-full">
            ...
            {!walletClient && (
                <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 dark:bg-yellow-900 dark:border-yellow-800 dark:text-yellow-200 px-4 py-3 rounded mb-4">
                    ...
                    <p className="mt-1">Please switch to the Registration tab to connect your wallet first.</p>
                </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                    ...
                    className={`w-full py-2 px-4 rounded-md text-white font-medium ${isLoading || !hasAccountCode || !walletClient
